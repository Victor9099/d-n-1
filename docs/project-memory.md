...
### Phase 1: DB & DevOps Foundation ✓

**Stories completed:** 12/12 (2 epics)
**Date:** 2026-08-17
**Deliverables:**

**Epic 2 — Database Foundation (6 stories):**

- Prisma schema: 19 models, 22 enums across 5 domains
  - Identity: User, StaffAccount, Session, OtpChallenge
  - Catalog: Category, Product, Variant (with stock management)
  - Cart/Checkout: CartItem, Quote, InventoryReservation
  - Order/Payment: Order, OrderLineItem, PaymentAttempt, FulfillmentEvent
  - Content/Media: Post, MediaAsset, MediaAttachment, SlugRedirect
  - Infrastructure: AuditLog, DomainEvent, ShippingConfig
- All constraints, indexes, unique rules applied
- Seed script (categories + admin user)
- Turbo tasks for db:generate, db:migrate, db:validate, db:seed
- Prisma client generated and validated ✅

**Epic 3 — DevOps Runtime (6 stories):**

- Docker Compose: PostgreSQL, Mailpit, VNPAY Simulator, API, Storefront, Admin
- Multi-stage Dockerfiles: non-root users, Alpine base, optimized builds
- VNPAY Callback Simulator: payment gateway mock with UI
- Health check scripts + wait-for-it dependency waiting
- Mailpit for local email capture (SMTP + Web UI)
- CI pipeline: quality gates, UBS scan, Prisma validation, contract checks, Docker build matrix

**Key decisions:**

- Polymorphic relations: separate FK columns (productId, postId) instead of single resourceId
- AuditLog.actorId: plain String without FK (polymorphic actor: customer/staff/system)
- Money fields: Int (VND), no separate currency column needed for MVP
- Cart: session-scoped (userId or sessionId), no standalone Cart table
- Address: embedded as columns on Order, not separate table
- DomainEvent: outbox pattern with ordering key for reliable event sourcing
...
_This file is project-specific. For universal lessons, use Cass Memory (`cm`)._
